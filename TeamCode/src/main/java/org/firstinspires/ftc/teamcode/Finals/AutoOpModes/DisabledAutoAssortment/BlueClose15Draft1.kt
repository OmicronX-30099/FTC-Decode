...
@Disabled()
@Autonomous(name="15 Ball", group="Tests")
class BlueClose15Draft1: NextFTCOpMode() {
    init {
        addComponents(
            LoopTimeComponent(),
            BulkReadComponent,
    ...
    fun buildPaths() {
        ...
        val thirdShoot = follower
            ...
            .setConstantHeadingInterpolation(Math.toRadians(180.0))
            .build()
        val rotateBot = follower
            .pathBuilder()
            .addPath(
                BezierLine(Pose(60.000, 23.375), Pose(8.000, 23.375))
            )
            .setLinearHeadingInterpolation(Math.toRadians(180.0), Math.toRadians(-90.0))
            .build()

        val intake15Ball = follower
            .pathBuilder()
            .addPath(BezierLine(Pose(8.000, 23.375), Pose(8.000, 7.600)))
            .setConstantHeadingInterpolation(Math.toRadians(-90.0))
            .build()

        val shoot15Ball = follower
            .pathBuilder()
            .addPath(
                BezierCurve(
                    Pose(8.000, 7.600),
